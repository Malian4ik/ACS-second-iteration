...
import { contactLinks, contactOptions, howItWorks, roomsNav } from "@/lib/site-data";

export const metadata: Metadata = {
  title: "Rooms | Avulus Cyber Space",
  description: "Browse Solo, Duo, and Bootcamp room formats and contact Avulus Cyber Space in one tap."
};
...
export default async function RoomsPage({ searchParams }: RoomsPageProps) {
  ...
  const requestedFilter = resolvedSearchParams?.format;
  const initialFilter =
  ...
  return (
    <div className="pb-24 md:pb-0">
      <AvulusNav ctaHref={contactLinks.booking} ctaLabel="Забронировать" items={roomsNav} />
      ...
      <main className="section-shell space-y-16 pt-32 md:pt-36">
        <section className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-end">
          <div>
            <div className="eyebrow">Rooms page</div>
            <h1 className="mt-4 max-w-4xl text-5xl font-semibold leading-[0.96] tracking-[-0.05em] text-white md:text-6xl">
              Choose the room format fast, then move straight into contact.
            </h1>
            <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-300">
              This page exists to remove uncertainty: clear room formats, visible pricing, and one-tap messenger actions.
            </p>
          </div>
          <div className="rounded-[32px] border border-white/10 bg-white/5 p-6">
            <div className="text-xs uppercase tracking-[0.3em] text-cyan-200">For longer stays</div>
            <p className="mt-3 text-base leading-7 text-slate-300">
              Push the restaurant as a support layer for duo and bootcamp bookings, not as a separate detour.
            </p>
            <TrackedLink
        ...
        <section>
          <RoomsExplorer initialFilter={initialFilter} />
        </section>
        ...
        <section className="rounded-[36px] border border-white/10 bg-[linear-gradient(135deg,rgba(20,10,34,0.92),rgba(8,22,38,0.92))] p-6 md:p-10" id="contact-options">
          <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
            <div>
              ...
              <h2 className="mt-4 text-4xl font-semibold tracking-[-0.04em] text-white">Close the loop while intent is still high.</h2>
              <p className="mt-5 max-w-xl text-sm leading-7 text-slate-300">
                The CTA layer should feel immediate. No heavy forms. No extra screens. Messenger, messenger, phone.
              </p>
            </div>
            <div className="grid gap-4 md:grid-cols-3">
              {contactOptions.map((option) => (
                ...
                >
                  ...
                  <div className="mt-5 text-sm font-semibold text-fuchsia-200">Open now</div>
                </TrackedLink>
              ))}
            </div>
          </div>
